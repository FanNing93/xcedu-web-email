    ...
    <div class="layout-list-content-search padding-top-size-nomal padding-left-size-nomal padding-right-size-nomal">
      <el-row :gutter="20" type="flex" justify="space-between">
        <el-col :span="8">
          <el-input v-model="searchFrom.searchName" class="input-with-select" placeholder="主题、发件人" clearable>
            <el-button slot="prepend" icon="el-icon-search" @click="searchEmail" />
            <el-button slot="append" type="primary" :icon="advanceSearchShow ? 'el-icon-arrow-up' : 'el-icon-arrow-down'" @click="showAdvanceSearch" />
          </el-input>
        </el-col>
      ...
      <el-form ref="form" :model="searchFrom" label-width="80px">
        <transition name="el-zoom-in-top">
          <el-row v-show="advanceSearchShow" :gutter="20" class="padding-top-size-nomal">
            <el-col :span="10">
      ...
      >
        <template slot-scope="scope">
          ...
          <div v-else-if="scope.column.property === 'oprate'" class="oprate-icon-groups">
            <i v-if="viewType === 'draftBox'" class="el-icon-edit" @click="editEmailById(scope.row.id)" />
            <i v-if="scope.row.status === 1" class="icon-undo" />
            <i v-if=" viewType === 'sendBox'" class="icon-view" />
            <i v-if="viewType !== 'draftBox' && viewType !== 'sendBox'" :class="scope.row.starIs === 1 ? 'icon-star-solid yellow' : 'icon-star-hollow'" style="cursor: pointer" @click="handleStar(scope.$index, scope.row)" />
